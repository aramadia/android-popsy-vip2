...
public:
	...
	void Move( Ufloat step )	{Ufloat tmp[3];	vec3_mul( tmp, dir, vitesse*step); vec3_add(pos,tmp,pos);}

	bool IsFinish()					{if( alpha<=0.0f ) return TRUE; else return FALSE;}
	void GetPos1( Ufloat dest[3])	{vec3_eg( dest, pos);}
	void GetPos2( Ufloat dest[3])	{vec3_mul( dest, dir, -size); vec3_add(dest,dest,pos);}
...
public:
	...
	void	GetPos( Ufloat dest[3])	{vec3_eg( dest, pos);}
