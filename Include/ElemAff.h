...
public:
	...
	int GetXSize()	{return dx;}
	int GetYSize()	{return dy;}

	bool Cmp( int ID )	{if(ID==IDBit)return TRUE;else return FALSE;}
...
public:
	...
	int		GetColorMask()	{return colormask;}
	int		GetID()			{return ID;}
	bool	GetEtat()		{return Etat;}

	void SetHighLight( bool b )	{Etat=b;}

	bool	IsInside( int color )	{if(color==colormask) return TRUE;else return FALSE;}
	bool	Cmp( int id )		{if(ID==id)return TRUE;else return FALSE;}
