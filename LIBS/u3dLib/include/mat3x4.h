...
public:
    ...
    void SetXPos(Ufloat x)                                 {
        xp=x;
    }
    void SetYPos(Ufloat y)                                 {
        yp=y;
    }
    void SetZPos(Ufloat z)                                 {
        zp=z;
    }
    void AddToPos(Ufloat x, Ufloat y, Ufloat z)                {
        xp+=x;
        yp+=y;
        zp+=z;
    }
...
//  Mat3x4& operator=(const Mat3x4& m);
    ...
    void    GetPos(Ufloat dest[3])                             {
        vec3_set(dest, xp, yp, zp);
    }
    float   GetXTransfo(const Ufloat v[3])                     {
        return v[0]*m11 + v[1]*m12 + v[2]*m13 + xp;
    }
    float   GetYTransfo(const Ufloat v[3])                     {
        return v[0]*m21 + v[1]*m22 + v[2]*m23 + yp;
    }
    float   GetZTransfo(const Ufloat v[3])                     {
        return v[0]*m31 + v[1]*m32 + v[2]*m33 + zp;
    }
    float   GetXTransfoR(const Ufloat v[3])                        {
        return v[0]*m11 + v[1]*m12 + v[2]*m13;   // sans deplacement ( pour un vecteur )
    }
    float   GetYTransfoR(const Ufloat v[3])                        {
        return v[0]*m21 + v[1]*m22 + v[2]*m23;   // sans deplacement ( pour un vecteur )
    }
    float   GetZTransfoR(const Ufloat v[3])                        {
        return v[0]*m31 + v[1]*m32 + v[2]*m33;   // sans deplacement ( pour un vecteur )
    }
