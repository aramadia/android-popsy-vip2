...
public:
    ...
    Viewport&   GetActuaViewport()                          {
        return ActuaViewport;
    }
    void        SetActuaViewport(const Viewport& view) {
        ActuaViewport=view;
    }
    ...
    void SetAlphaRef(Ufloat val)                   {
        AlphaRef=val;   // met une nouvelle valeur dans l'alpha reference
    }
    void SortTransparentPoly(bool b)               {
        SortPolyTr=b;
    }
...
void inline PolyPipeLine::AddPolySpecular(PolygonU3D* p) {
    ...
    case MatShadingFlat:
        if ((p->RVBA[0]>1.0f) || (p->RVBA[1]>1.0f) || (p->RVBA[2]>1.0f)) {
            ...
            np->RVBA[0] -= 1.0f;
            if (np->RVBA[0]<0.0f) np->RVBA[0] = 0.0f;
            np->RVBA[1] -= 1.0f;
            if (np->RVBA[1]<0.0f) np->RVBA[1] = 0.0f;
            np->RVBA[2] -= 1.0f;
            if (np->RVBA[2]<0.0f) np->RVBA[2] = 0.0f;
        } else {
            return;
        }
        break;
    ...
    case MatShadingGouraud:
        ...
        for (a=0; a<np->nbVertex; a++,V++) {
            (*V)->c->RVBA[0] -= 1.0f;
            if ((*V)->c->RVBA[0]<0.0f)(*V)->c->RVBA[0] = 0.0f;
            (*V)->c->RVBA[1] -= 1.0f;
            if ((*V)->c->RVBA[1]<0.0f)(*V)->c->RVBA[1] = 0.0f;
            (*V)->c->RVBA[2] -= 1.0f;
            if ((*V)->c->RVBA[2]<0.0f)(*V)->c->RVBA[2] = 0.0f;
        }
        break;
