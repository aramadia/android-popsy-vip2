...
public:
    ...
    void SetTarg(Ufloat p[3])   {
        vec3_eg(Targ,p);
    }
    void SetRVB(Ufloat p[3])    {
        vec3_eg(RVB,p);
    }
    void SetIntensity(Ufloat i) {
        Intensity = i;
    }
    void SetAttenuation(bool b) {
        Attenuation=b;
    }
    void SetHotspot(Ufloat h)   {
        Hotspot=h;
    }
    void SetFalloff(Ufloat f)   {
        Falloff=f;
    }
    ...
    void    GetPos(Ufloat p[3])     {
        vec3_eg(p,Pos);
    }
    void    GetRVB(Ufloat dest[3])  {
        vec3_eg(dest, RVB);
    }
    Ufloat  GetIntensity()          {
        return Intensity;
...
struct Lumel {
    ...
    Lumel() {
        vec3_set(RVB, 0.f, 0.f, 0.f);
    }
    void Add(Ufloat rvb[3]) {
        vec3_add(RVB, RVB, rvb);
    }
    void Conv(U8 dest[3]);
